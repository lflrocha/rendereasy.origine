...
The Indicadores content type
===============================

In this section we are tesing the Indicadores content type by performing
basic operations like adding, updadating and deleting Indicadores content
items.

Adding a new Indicadores content item
--------------------------------
...
Then we select the type of item we want to add. In this case we select
'Indicadores' and click the 'Add' button to get to the add form.

    >>> browser.getControl('Indicadores').click()
    >>> browser.getControl(name='form.button.Add').click()
    >>> 'Indicadores' in browser.contents
    True

Now we fill the form and submit it.

    >>> browser.getControl(name='title').value = 'Indicadores Sample'
    >>> browser.getControl('Save').click()
    >>> 'Changes saved' in browser.contents
    True

And we are done! We added a new 'Indicadores' content item to the portal.

Updating an existing Indicadores content item
---------------------------------------

Let's click on the 'edit' tab and update the object attribute values.

    >>> browser.getLink('Edit').click()
    >>> browser.getControl(name='title').value = 'New Indicadores Sample'
    >>> browser.getControl('Save').click()
...
We check that the changes were applied.
    ...
    >>> 'Changes saved' in browser.contents
    True
    >>> 'New Indicadores Sample' in browser.contents
    True

Removing a/an Indicadores content item
--------------------------------

If we go to the home page, we can see a tab with the 'New Indicadores
Sample' title in the global navigation tabs.
...
Now we are going to delete the 'New Indicadores Sample' object. First we
go to the contents tab and select the 'New Indicadores Sample' for
deletion.

    >>> browser.getLink('Contents').click()
    >>> browser.getControl('New Indicadores Sample').click()
...
So, if we go back to the home page, there is no longer a 'New Indicadores
Sample' tab.
...
Adding a new Indicadores content item as contributor
------------------------------------------------

Not only site managers are allowed to add Indicadores content items, but
also site contributors.
...
We select 'Indicadores' and click the 'Add' button to get to the add form.
...
Done! We added a new Indicadores content item logged in as contributor.
